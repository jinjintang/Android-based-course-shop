...
import Users from '../api/users.js'
  export default {
    ...
    methods:{ 
      ...
      pass(index) {
        var scope=this;
           Users.approveResult({   data:{
               STATE:index,
           
              
            },Aid:this.Aid}, (ret, err) => {
          if (err) {
            console.log(err)
          }else{
...
if(index==1){
IndexApi.areas({id:this.Aid},(ret, err) => {
          if (err) {
            console.log(err)
          }else{
            this.Pid = ret.data.PARENTID
            
           // this.getCourseItem(ret.data.id)
          }
        }) 
if(scope.Pid!=0){
  scope.courseInfoData.STATE=scope.courseInfoData.STATE+1;
if(scope.courseInfoData.STATE==3)scope.courseInfoData.STATE=3;
       Users.approveResult2({   data:{
             data:{
              COURSEID:scope.courseInfoData.Id+'',
              APAREAID:scope.Pid+'',
              STATE:0
            },
...
else{scope.courseInfoData.STATE=3;}
IndexApi.courses2({data:scope.courseInfoData},(ret, err) => {
          if (err) {
            console.log(err)
          }else{
