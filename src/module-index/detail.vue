<template>
  <div class="learingIndexDetail">
    ...
    <div class="payCourse" v-if="is_buy==null "><span style="padding-left: 30px;">￥ {{courseInfoData.COURSEPRICE}}</span><router-link :to="{'path':'/pay/' + courseInfoData.Id}">立即购买</router-link></div>
    <div v-else class="startLearing">已报名</div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods:{
      // 初始化数据
      init: function() {
        IndexApi.getcourses({id:this.$route.params.classId},(ret, err) => {
          if (err) {
            console.log(err)
          }else{
