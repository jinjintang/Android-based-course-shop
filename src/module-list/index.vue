    ...
    methods:{
        ...
        if (obj == 'type') {
          IndexApi.listItem({parent_id: ''},(ret, err) => {
            if (err) {
              ...
              this.actions.push({Id:'0',CATEGORYNAME:'全部分类'})
           ...
           IndexApi.rankItem({parent_id: ''},(ret, err) => {
            if (err) {
              ...
              this.actions = ret.data
              this.actions.push({Id:'0', RANKNAME:'全部等级'})
            }
          })
        }
      },
      ...
      selectItem:function (obj, id) {
        if(this.sheetVisible == 'type' || this.sheetVisible == ''){
          this.typeTitle =  obj
          if(id!='0')this.subParams.query = 'CATEGORYID:'+id
        }else{
          this.rankTitle =  obj
           if(id!='0')this.subParams.query = 'RANKID:'+id
        }
        this.noData = false
        this.getCoursesItem()
        this.sheetVisible = ''
