<template>
  <div class="register">
    ...
    <mt-field label="姓名" :value=myInfoData.USERNAME placeholder="请输入姓名" v-model="myInfoData.USERNAME" 
   ></mt-field>
      ...
      <!-- <p><mt-field label="" placeholder="验证码" :state="telCodeStat" disableClear type="telCode"></mt-field>
       ...
       <mt-field label="邮箱" placeholder="请输入邮箱"type="email"  v-model="myInfoData.EMAIL"></mt-field>
        <mt-field label="地址" placeholder="请输入地址" v-model="myInfoData.ADRESS"></mt-field>
        ...
        // if (this.phonStat == 'success' ) {
          ...
          Users.register(this.myInfoData,(ret, err) => {
            if (err) {
              ...
              localStorage.setItem('user',this.myInfoData)
            }
          })
        // } else {
